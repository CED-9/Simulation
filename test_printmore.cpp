...
void singleSimulation(
    ...
    int window_size,
    double* resultRate, double* resultReps)
{
    // config the network
    ...
    *resultRate = failRateTotal / (cnt + 2.0 * window_size + 1.0);
    ...
    lock_reps.lock();
    *resultReps = cnt + 2.0 * window_size + 1.0;
    lock_reps.unlock();
    /*lock_reps.lock();
    lock_reps.unlock();
    lock_cout.lock();
...
int main(int argc, char* argv[]){
	...
	for (int i = 0; i < 10; ++i){
        ...
        for (int j = 0; j < iter; j++) {
            cout << (double)(threshold*199) << " " << rates[j] << " " << reps[j];
            rateFinal += rates[j];
        }
