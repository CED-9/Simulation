...
// initial the network before run
// order: pro -> con -> fin -> ban&lab
Status Graph::init(){
...
// update all notes' info about the graph
// producer list price,
// consumer collect info,
// financial collect info,
// bank, labor check policy
Status Graph::update(){
	// update con, pro, fin
    for (int i = 0; i < this->proNum; i++) {
...
// Producer pay back labor
Status Graph::proPayLab(ProNode* p){
	...
	double profit = p->last_sale - value;
	...
	double trueValue;
	payCase2(node1, node2, value, trueValue);
	l->paymentIn(trueValue);

	return GOOD;
}

// Labor pay to consumers
Status Graph::labPayCon(){
	LabNode* l = this->labAgent;
	double unit_payment = l->getCurrPayment() / (this->conNum);
	if (unit_payment < 0.000001){
		unit_payment = 0;
	}
	...
	for (int i = 0; i<this->conNum; i++) {
		...
		this->conAgent[i]->setLastIncome(unit_payment);
	}

	return GOOD;
}

// All cons nodes will get opertunities from all pros on list
Status Graph::genTrans(){
...
Status Graph::genCostAndDivPay(){
    Status status;
    for (int i=0; i<this->proNum; i++) {
        //cout<<"proAgent ID: "<<this->proAgent[i]->getNodeID()<<endl;
        status = this->proPayLab(this->proAgent[i]);
    }
    status = this->labPayCon();
    return GOOD;
}

void Graph::debtCancel(){
	// Consumer
	for (int i = 0; i < this->conNum; i++){
...
Status Graph::chargeIR(int time){
	// if (time == 10){
...
void Graph::executeDefault(FinNode* f, int time){
